...
param(
    ...
    [switch]$LocalControllerTestsOnly,
...
$primaryTestSelectors = @(
    ...
    [pscustomobject]@{ Name = 'LocalControllerTestsOnly'; Selected = [bool]$LocalControllerTestsOnly }
...
$promptFontOutput = Join-Path $outputDirectory 'assets\fonts\promptfont'
New-Item -ItemType Directory -Force -Path $promptFontOutput | Out-Null
...
function Invoke-LocalControllerOwnerTests {
    ...
    $arguments = $common + @(
        ...
        (Join-Path $platformDirectory 'HidHideConfigurationAdapter.cpp'),
        "/Fo:$controllerIsolationTestObjectDirectory\",
        "/Fe:$outputDirectory\LocalControllerOwnerTests.exe", '/link', '/SUBSYSTEM:CONSOLE'
    ) + $libraryArguments + @('shell32.lib', 'ole32.lib')
    & $cl $arguments
    if ($LASTEXITCODE -ne 0) { throw "LocalControllerOwnerTests compile failed: $LASTEXITCODE" }
...
function Invoke-ControllerIsolationLocalTests {
    Invoke-LocalControllerOwnerTests
    $cases = @(
        [pscustomobject]@{
...
if ($LocalControllerTestsOnly) {
    if ($SkipTests) { throw 'LocalControllerTestsOnly cannot be combined with SkipTests.' }
    Invoke-LocalControllerOwnerTests
    return
}
