    ...
    internal static bool IsValidValue(PresentationPropertyChange change)
    {
        try
        {
            object? parsed = change.Property switch
            {
                ...
                PresentationProperty.ContextMenuButton => Read<ControllerButton?>(change.Value),
                ...
                PresentationProperty.CollectionResetGeneration => Read<long?>(change.Value),
        ...
        IReadOnlyList<PresentationPropertyChange> changes)
    {
        foreach (var change in changes)
            node = change.Property switch
            {
                ...
                PresentationProperty.ContextMenuButton => node with { ContextMenuButton = Read<ControllerButton?>(change.Value) },
                ...
                PresentationProperty.ShowScrollbar => node with { ShowScrollbar = Read<bool?>(change.Value) },
