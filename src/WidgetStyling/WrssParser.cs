...
public static partial class WrssParser
{
    private static readonly IReadOnlyDictionary<string, WrssPseudoState> PseudoStates =
        new Dictionary<string, WrssPseudoState>(StringComparer.Ordinal)
        {
            ["focused"] = WrssPseudoState.Focused,
            ["pressed"] = WrssPseudoState.Pressed,
            ["selected"] = WrssPseudoState.Selected,
            ["disabled"] = WrssPseudoState.Disabled,
            ["busy"] = WrssPseudoState.Busy,
        };

    public static WrssParseResult Parse(string source, string sourceName = "<memory>")
    {
        ArgumentNullException.ThrowIfNull(source);
        ArgumentException.ThrowIfNullOrWhiteSpace(sourceName);
        var diagnostics = new List<WrssDiagnostic>();
        if (source.Length > WrssLimits.MaximumSourceCharacters)
        {
            diagnostics.Add(Diagnostic(sourceName, source, WrssLimits.MaximumSourceCharacters,
                "source_too_large", $"A WRSS source may contain at most {WrssLimits.MaximumSourceCharacters} characters."));
            return new WrssParseResult(new WrssDocument(sourceName, []), diagnostics);
        }
        var content = RemoveComments(source, sourceName, diagnostics);
        var statements = new List<WrssStatement>();
        var index = 0;
        var sawRule = false;
        var importCount = 0;
        ...
        while (true)
        {
            ...
            if (statements.Count >= WrssLimits.MaximumStatements)
            {
                Add(index, "too_many_statements", $"A WRSS source may contain at most {WrssLimits.MaximumStatements} statements.");
                break;
            }
            var start = index;
            if (content[index] == '@')
            {
                ...
                else
                {
                    ...
                    if (!WrssPackageLoader.IsSafePackagePath(path))
                        Add(start, "unsafe_import", "Import must be a normalized package-relative .wrss path without traversal or a URI scheme.");
                    else if (++importCount > WrssLimits.MaximumImports)
                        Add(start, "too_many_imports", $"A WRSS source may import at most {WrssLimits.MaximumImports} files.");
                    else
                        statements.Add(new WrssImport(path, Location(sourceName, content, start)));
                }
                index = end + 1;
                continue;
            }
            ...
            if (selectors.Count != 0)
                statements.Add(new WrssRule(selectors, declarations, Location(sourceName, content, start)));
            sawRule = true;
            index = close < content.Length ? close + 1 : close;
        }

        return new WrssParseResult(new WrssDocument(sourceName, statements), diagnostics);

        void Add(int offset, string code, string message) =>
            diagnostics.Add(Diagnostic(sourceName, content, offset, code, message));
    }

    private static IReadOnlyList<WrssSelector> ParseSelectors(
        string source,
        string sourceName,
        string fullSource,
        int offset,
        List<WrssDiagnostic> diagnostics)
    {
        var selectors = new List<WrssSelector>();
        if (string.IsNullOrWhiteSpace(source))
        {
            diagnostics.Add(Diagnostic(sourceName, fullSource, offset, "missing_selector", "A rule requires a selector."));
            return selectors;
        }
        if (source.Length > WrssLimits.MaximumSelectorCharacters)
        {
            diagnostics.Add(Diagnostic(sourceName, fullSource, offset, "selector_too_long",
                $"A selector list may contain at most {WrssLimits.MaximumSelectorCharacters} characters."));
            return selectors;
        }

        var rawSelectors = source.Split(',');
        if (rawSelectors.Length > WrssLimits.MaximumSelectorsPerRule)
        {
            diagnostics.Add(Diagnostic(sourceName, fullSource, offset, "too_many_selectors",
                $"A rule may contain at most {WrssLimits.MaximumSelectorsPerRule} selectors."));
            return selectors;
        }
    ...
    private static bool TryParseSelector(string text, out WrssSelector? selector, out string error)
    {
        selector = null;
        error = string.Empty;
        if (text == ":root")
        {
            selector = new WrssSelector(null, null, [], new HashSet<WrssPseudoState>(), true, 0, text);
            return true;
        }
        ...
        selector = new WrssSelector(role, id, classes, states, false, specificity, text);
        return true;
    }

    private static IReadOnlyList<WrssDeclaration> ParseDeclarations(
        string block,
        string sourceName,
        string fullSource,
        int blockOffset,
        List<WrssDiagnostic> diagnostics)
    {
        var declarations = new List<WrssDeclaration>();
        ...
        for (var index = 0; index <= block.Length; index++)
        {
            ...
            if (++declarationCount > WrssLimits.MaximumDeclarationsPerRule)
            {
                diagnostics.Add(Diagnostic(sourceName, fullSource, declarationOffset, "too_many_declarations",
                    $"A rule may contain at most {WrssLimits.MaximumDeclarationsPerRule} declarations."));
                break;
            }
            ...
            if (value.Length > WrssLimits.MaximumValueCharacters)
            {
                diagnostics.Add(Diagnostic(sourceName, fullSource, declarationOffset + colon + 1, "value_too_long",
                    $"A value may contain at most {WrssLimits.MaximumValueCharacters} characters."));
                continue;
            }
            if (!CustomPropertyRegex().IsMatch(property) && !WrssPropertyCatalog.IsAllowed(property))
            ...
            if (ContainsUnsafeValue(value))
                diagnostics.Add(Diagnostic(sourceName, fullSource, declarationOffset + colon + 1, "unsafe_value", "URLs, scripts, expressions, imports, and filesystem values are not allowed in WRSS declarations."));
            ...
            declarations.Add(new WrssDeclaration(property, value, Location(sourceName, fullSource, declarationOffset), declarations.Count));
        }
        if (quote != '\0')
    ...
    private static string RemoveComments(string source, string sourceName, List<WrssDiagnostic> diagnostics)
    {
    ...
    internal static WrssSourceLocation Location(string sourceName, string source, int offset)
    {
        var (line, column) = GetLineColumn(source, offset);
        return new WrssSourceLocation(sourceName, line, column);
    }

    internal static WrssDiagnostic Diagnostic(string sourceName, string source, int offset, string code, string message,
        WrssDiagnosticSeverity severity = WrssDiagnosticSeverity.Error)
    {
        var location = Location(sourceName, source, offset);
        return new WrssDiagnostic(location.Source, location.Line, location.Column, severity, code, message);
    }
