...
public static partial class WrssPropertyCatalog
{
    ...
    private static readonly IReadOnlyDictionary<string, Definition> Definitions =
        new Dictionary<string, Definition>(StringComparer.Ordinal)
        {
            ...
            ["border-right-color"] = new(PropertyType.Color),
            ["border-bottom-color"] = new(PropertyType.Color),
            ["border-left-color"] = new(PropertyType.Color),
            ...
            ["scrollbar-track-color"] = new(PropertyType.Color),
            ["scrollbar-thumb-color"] = new(PropertyType.Color),
            ["scrollbar-width"] = new(PropertyType.Length, 2, 8),
            ...
            ["border-top-width"] = new(PropertyType.Length, 0, 16),
            ["border-right-width"] = new(PropertyType.Length, 0, 16),
            ["border-bottom-width"] = new(PropertyType.Length, 0, 16),
            ["border-left-width"] = new(PropertyType.Length, 0, 16),
    ...
    internal static bool TryCompute(
        string property,
        string value,
        out WrssComputedValue? computed,
        out bool clamped,
        out string error)
    {
        computed = null;
        ...
        switch (definition.Type)
        {
            ...
            case PropertyType.Scale:
                ...
                computed = new WrssComputedValue(WrssValueKind.Number, Format(bounded), bounded);
                return true;
            case PropertyType.Duration:
                ...
                computed = new WrssComputedValue(WrssValueKind.Duration, $"{Format(duration)}ms", duration, "ms");
                return true;
            case PropertyType.FontWeight:
                if (value is "normal" or "bold")
                {
                    computed = new WrssComputedValue(WrssValueKind.Keyword, value);
                    return true;
                }
                ...
                computed = new WrssComputedValue(WrssValueKind.Number, weight.ToString(CultureInfo.InvariantCulture), weight);
                return true;
            case PropertyType.FontFamily:
                ...
                computed = new WrssComputedValue(WrssValueKind.FontFamily, NormalizeWhitespace(value));
                return true;
            ...
            case PropertyType.LineHeight:
                ...
                computed = new WrssComputedValue(WrssValueKind.Number, Format(lineHeight), lineHeight);
                return true;
            case PropertyType.PositiveInteger:
                ...
                computed = new WrssComputedValue(WrssValueKind.Integer, boundedInteger.ToString(CultureInfo.InvariantCulture), boundedInteger);
                return true;
            ...
            case PropertyType.BoundedNumber:
                ...
                computed = new WrssComputedValue(WrssValueKind.Number, Format(flexFactor), flexFactor);
                return true;
            ...
            default:
                throw new InvalidOperationException("Unknown WRSS property type.");
        }
    }
        ...
        out string error)
    {
        ...
        computed = new WrssComputedValue(WrssValueKind.Length, $"{Format(number)}{unit}", number, unit);
        return true;
    }
        ...
        out string error)
    {
        ...
        computed = new WrssComputedValue(WrssValueKind.LengthList, string.Join(' ', normalized));
        return true;
    }
        ...
        out string error)
    {
        ...
        computed = new WrssComputedValue(WrssValueKind.Ratio, Format(ratio), ratio);
        return true;
    }
    ...
    private static bool TryKeyword(string value, IReadOnlyList<string> allowed, out WrssComputedValue? computed, out string error)
    {
