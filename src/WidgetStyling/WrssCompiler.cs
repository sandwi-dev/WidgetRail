...
public static class WrssThemeCompiler
{
    public static WrssCompileResult Compile(WrssPackageResult package, WrssCompileOptions? options = null)
    {
    ...
    public static WrssCompileResult Compile(
        IEnumerable<WrssDocument> documents,
        WrssCompileOptions? options = null)
    {
        ArgumentNullException.ThrowIfNull(documents);
        return Compile([new WrssThemeLayer(0, documents.ToArray())], options);
    }

    public static WrssCompileResult Compile(
        IEnumerable<WrssThemeLayer> layers,
        WrssCompileOptions? options = null)
    {
        ArgumentNullException.ThrowIfNull(layers);
        options ??= new WrssCompileOptions();
        ...
        foreach (var (name, value) in options.BuiltinVariables.OrderBy(item => item.Key, StringComparer.Ordinal))
            variables[name] = new VariableDefinition(value, new WrssSourceLocation("<host>", 1, 1));

        foreach (var sourceDocument in sourceDocuments)
        {
            foreach (var rule in sourceDocument.Document.Statements.OfType<WrssRule>())
            {
        ...
        var compiledRules = new List<WrssTheme.CompiledRule>();
        var cascadeOrder = 0;
        foreach (var sourceDocument in sourceDocuments)
        {
            foreach (var rule in sourceDocument.Document.Statements.OfType<WrssRule>())
            {
                ...
                var declarations = new List<WrssTheme.CompiledDeclaration>();
                foreach (var declaration in rule.Declarations)
                {
                    if (declaration.Property.StartsWith("--", StringComparison.Ordinal)) continue;
                    if (!WrssPropertyCatalog.IsAllowed(declaration.Property)) continue;
                    ...
                    if (!WrssPropertyCatalog.TryCompute(declaration.Property, value, out var computed, out var clamped, out var valueError))
                    {
                        Add(declaration.Location, "invalid_value", $"{declaration.Property}: {valueError}");
                        continue;
                    }
                    if (clamped)
                        Add(declaration.Location, "value_clamped", $"{declaration.Property} was clamped to {computed!.Text}.", WrssDiagnosticSeverity.Warning);
                    declarations.Add(new WrssTheme.CompiledDeclaration(declaration.Property, computed!, declaration.Order));
                }
                compiledRules.Add(new WrssTheme.CompiledRule(
        ...
        var hasErrors = diagnostics.Any(item => item.Severity == WrssDiagnosticSeverity.Error);
        return new WrssCompileResult(hasErrors ? null : new WrssTheme(compiledRules), diagnostics);
        ...
        void Add(WrssSourceLocation location, string code, string message, WrssDiagnosticSeverity severity = WrssDiagnosticSeverity.Error) =>
            diagnostics.Add(new WrssDiagnostic(location.Source, location.Line, location.Column, severity, code, message));
    }
        ...
        out string error)
    {
        ...
        for (var index = 0; index < source.Length;)
        {
            ...
            if (varIndex < 0)
            {
                if (output.Length + source.Length - index > WrssLimits.MaximumExpandedValueCharacters)
                {
                    result = string.Empty;
                    error = $"Expanded value exceeds {WrssLimits.MaximumExpandedValueCharacters} characters.";
                    return false;
                }
                output.Append(source, index, source.Length - index);
                break;
            }
            if (output.Length + varIndex - index > WrssLimits.MaximumExpandedValueCharacters)
            {
                result = string.Empty;
                error = $"Expanded value exceeds {WrssLimits.MaximumExpandedValueCharacters} characters.";
                return false;
            ...
            if (output.Length + replacement.Length > WrssLimits.MaximumExpandedValueCharacters)
            {
                result = string.Empty;
                error = $"Expanded value exceeds {WrssLimits.MaximumExpandedValueCharacters} characters.";
                return false;
    ...
    private sealed record VariableDefinition(string Value, WrssSourceLocation Location);
    private sealed record LayeredDocument(WrssDocument Document, int LayerPriority);
}
