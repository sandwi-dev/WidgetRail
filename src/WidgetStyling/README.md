...
`WidgetStyling` is the shared parser, validator, package loader, typed property catalog, variable resolver, and deterministic cascade for WRSS. Developer tooling and the future renderer should consume this assembly instead of maintaining separate CSS-like parsers.

## Integration API

```csharp
var package = WrssPackageLoader.LoadFile(
    entryFile: @"C:\Widget\styles\default.wrss",
...
if (compiled.IsValid)
{
    var style = compiled.Theme!.Resolve(new WrssElement(
        ...
        PseudoStates: new HashSet<WrssPseudoState> { WrssPseudoState.Focused }));
...
- Quoted package-relative `.wrss` imports before rules, with depth/cycle/missing-file checks.
...
The allowlist is available as `WrssPropertyCatalog.AllowedProperties`. It covers layout, typography, colors, borders/outlines, opacity, scale, blur/shadow geometry, and transition duration; unsupported properties are errors.
...
value of a two-value `gap`. Image source selection remains outside WRSS and
must come from verified host content.

## Deliberate safety boundary

WRSS is not CSS and has no browser escape hatch. It rejects `url()`, `http:`, `https:`, `file:`, `data:`, JavaScript/VBScript, `expression()`, script/eval functions, declaration-level `@` statements, selector combinators, attribute selectors, and unknown functions/values. Imports cannot be absolute, contain backslashes/schemes, or use `.`/`..` segments. There is no shader, command, native-extension, or arbitrary filesystem syntax.
...
Accessibility rules are not represented as WRSS. The host applies text scaling, high contrast, focus visibility, reduced motion, and other mandatory accessibility overrides after resolving the theme, so author styles cannot defeat them.
