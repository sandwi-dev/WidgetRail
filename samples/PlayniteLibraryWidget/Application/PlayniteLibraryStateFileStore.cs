...
internal sealed class PlayniteLibraryStateFileStore
{
    ...
    internal PlayniteLibraryStateFileStore(string path)
    {
    ...
    internal async ValueTask<WidgetPrivateStateValue<PlayniteLibraryPrivateState>> ReadAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (stateBytes.Length > MaximumStateBytes)
            throw new InvalidDataException("Playnite Library organization exceeds 64 KiB.");
        ...
        try
        {
            ...
            if (expectedRevision is { } expected && expected != revision)
                throw new WidgetCapabilityException(
                    "state_conflict", "Playnite Library organization changed concurrently.");
            if (revision == long.MaxValue)
                throw new InvalidDataException("Playnite Library state revision is exhausted.");
    ...
    private async Task<StateDocument?> LoadAsync(CancellationToken cancellationToken)
    {
        ...
        if (stream.Length is <= 0 or > MaximumEnvelopeBytes)
            throw new InvalidDataException("Playnite Library organization is invalid.");
    ...
    private sealed record StateDocument(
        ...
        [property: JsonRequired] PlayniteLibraryPrivateState State);
}
