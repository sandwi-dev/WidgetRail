...
if ($LASTEXITCODE -ne 0) {
    throw "Game Launcher application publish failed with exit code $LASTEXITCODE."
}
...
if ($forbidden.Count -ne 0) {
    throw "Staged Game Launcher contains product-owned/debug files: [$($forbidden -join ', ')]."
}
...
if ($Install) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Game Launcher install failed.' }
    & dotnet run --project $cliProject --configuration $Configuration `
        --no-launch-profile -- version select $manifest.id $manifest.version @catalogArguments
    if ($LASTEXITCODE -ne 0) { throw 'Game Launcher version selection failed.' }
    & dotnet run --project $cliProject --configuration $Configuration `
        --no-launch-profile -- enable $manifest.id --accept-full-trust @catalogArguments
    if ($LASTEXITCODE -ne 0) { throw 'Game Launcher enable failed.' }
}

Write-Host "Community Game Launcher package: $packagePath"
