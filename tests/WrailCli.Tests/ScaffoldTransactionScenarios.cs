...
internal static class ScaffoldTransactionScenarios
{
    ...
    private sealed class ScaffoldFixture : IDisposable
    {
        public ScaffoldFixture(string name)
        {
            Root = Path.Combine(
                Path.GetTempPath(),
                $"wrail-scaffold-{name}-{Guid.NewGuid():N}");
        ...
        {
            SaveManifest();
            var original = Environment.GetEnvironmentVariable("WRAIL_TEMPLATE_ROOT");
            Environment.SetEnvironmentVariable("WRAIL_TEMPLATE_ROOT", TemplateRoot);
            try
            {
            ...
            {
                Environment.SetEnvironmentVariable("WRAIL_TEMPLATE_ROOT", original);
            }
        }
        ...
        public void AssertNoStagingResidue()
        {
            var residues = Directory.EnumerateFileSystemEntries(
                    Root, ".*.wrail-stage-*", SearchOption.TopDirectoryOnly)
                .ToArray();
