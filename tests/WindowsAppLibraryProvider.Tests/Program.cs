...
var allTests = new (string Name, Func<Task> Run)[]
{
    ...
    ("Running packaged identity interop is exact wide and bounded",
        RunningAppScenarios.PackagedIdentityInteropIsExactWideAndBounded),
    ("Switcher window eligibility handles helpers owners popups and minimized windows",
        RunningWindowScenarios.EligibilityAndOwnership),
    ("Hosted app windows resolve a unique live packaged identity",
        RunningWindowScenarios.HostedAppIdentity),
    ("Disappearing inaccessible and unsupported windows are skipped individually",
        RunningWindowScenarios.FailureIsolation),
    ("Running window class interop is exact wide and bounded",
        RunningAppScenarios.WindowClassInteropIsExactWideAndBounded),
    ("Running native callback defers managed failures across the boundary",
        RunningAppScenarios.NativeCallbackDefersManagedFailure),
    ("Running process context keeps same-user session without elevation rejection",
        RunningAppScenarios.ProcessContextDoesNotRejectElevation),
    ...
    ("Portable launch is direct by default and UAC consent fallback is exact",
