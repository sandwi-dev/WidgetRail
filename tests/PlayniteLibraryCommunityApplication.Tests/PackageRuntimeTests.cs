...
public sealed class PackageRuntimeTests
{
    ...
    public async Task PackageServicePowersCollectionsDetailsBackAndRestartState()
    {
        ...
        var widget = WidgetTestHost.Attach(
            new PlayniteLibraryWidget(service),
        ...
        await widget.OnActionAsync(new(
            "playnite-library.favorite", "playnite-library.details.launch"));
        ...
        var favorite = Nodes(returned.Root).Single(node =>
            node.ActionId == PlayniteLibraryCollectionPolicy.ActionPrefix + "favorites");
        ...
        Assert.AreEqual(tile.CollectionItemKey,
            PlayniteLibraryIdentity.Key(current.Items[0].SavedId).Value);
    }
    ...
    public async Task PackageUpdateReplacementAndReinstallRetainUserStateAndSavedIdentity()
    {
        ...
        await using (var original = Service(directory.Path, new PackageSource(1)))
        {
            ...
            var state = PlayniteLibraryPrivateState.Empty with
            {
    ...
    public async Task PlayniteConnectionRouteNeverPublishesTokenAndRejectsLateCompletion()
    {
        ...
        var widget = WidgetTestHost.Attach(
            new PlayniteLibraryWidget(service, client),
            new WidgetTestHostServicesBuilder().Build());
        ...
        Assert.AreEqual("playnite-library.playnite.token", setup.InitialFocusId);
        var entry = Nodes(setup.Root).Single(node =>
            node.Id == "playnite-library.playnite.token");
        ...
        await widget.OnActionAsync(new(
            ...
            "playnite-library.playnite.token") { CommittedText = sentinel });
        ...
        var late = widget.OnActionAsync(new(
            ...
            PlayniteLibraryWidget.PlayniteRefreshActionId)).AsTask();
        ...
        PackageSource source)
    {
        ...
        return new(
            provider,
            new PlayniteLibrarySavedIdIssuer(Path.Combine(root, "saved-id.key")),
            new PlayniteLibraryStateFileStore(Path.Combine(root, "organization.json")));
    }
    ...
    private static ViewSnapshot Snapshot(PlayniteLibraryWidget widget, long sequence)
    {
