...
internal static class LauncherThemeScenarios
{
    ...
    {
        using var temp = new TestDirectory();
        var installation = RequiredEnvironment("GBA_LAUNCHER_LIFECYCLE_INSTALLATION");
        var hostTest = RequiredEnvironment("GBA_LAUNCHER_LIFECYCLE_HOST_TEST");
        var fixtureBridge = RequiredEnvironment("GBA_LAUNCHER_LIFECYCLE_FIXTURE_BRIDGE");
        var bottom = Path.Combine(temp.Path, "bottom-rail");
        var left = Path.Combine(temp.Path, "left-rail");
        var bottomArchive = Path.Combine(temp.Path, "bottom-rail.gbarlauncher");
        var leftArchive = Path.Combine(temp.Path, "left-rail.gbarlauncher");
        var bottomPreview = Path.Combine(temp.Path, "bottom-preview.json");
        var leftPreview = Path.Combine(temp.Path, "left-preview.json");
        var settingsRoot = Path.Combine(temp.Path, "local-app-data", "GameBarAlternative");
        ...
        Equal("org.gbar.builtin.hero-rail", finalSettings.LauncherExperience.SelectedId);
