...
| `src/WindowsAppLibraryProvider` | Lazy bounded Start Menu, current-user AppsFolder, supported installed Microsoft/Xbox package-game, and Steam sources behind one normalized private source contract. Microsoft/Xbox Game classification requires exact bounded `MicrosoftGame.config` evidence for the registered application; package paths, AUMIDs, configuration data, arguments, PIDs, and raw identities never enter widget payloads. Short-lived launch IDs and authority-scoped durable SavedIds remain opaque. A separately consented on-demand running-window observation maps visible unelevated programs only to an exact current registration. Launch revalidates the owning shortcut/package/AUMID/manifest immediately before constrained activation. |
