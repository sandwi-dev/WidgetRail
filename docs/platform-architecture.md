...
| `src/WindowsAppLibraryProvider` | Lazy bounded Start Menu plus current-user AppsFolder catalog on a process-wide Shell STA lane, with sanitized names, deduplication, short-lived launch IDs, host-only identities, and authority-scoped durable SavedIds. A separately consented on-demand running-window observation maps visible unelevated programs only to an exact current normalized registration and returns no process/window identity. Launch re-enumerates the exact shortcut or canonical AUMID before constrained Shell open/null-argument packaged activation; paths, AUMIDs, arguments, PIDs, and raw identities never enter widget payloads. |
