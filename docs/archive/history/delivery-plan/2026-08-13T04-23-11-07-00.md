...
This is immutable historical evidence captured when the active delivery plan
crossed its 1,000-physical-line compaction threshold. It is not implementation
authority. Use the current [delivery plan](../../delivery-plan.md).

# Delivery plan

Status: reviewer-owned production and retired managed UI prototype-evaluation execution queue,
2026-08-13 00:00 -07:00

Planning owner: independent review and delivery-planning agent

Execution owners: widgets, platform, and isolated retired managed UI prototype prototype

This file is the sole authority for implementation selection. The complete
pre-compaction state is preserved in
[the 2026-08-12 21:06 snapshot](../../../history/delivery-plan/history/delivery-plan/2026-08-12T21-06-00-07-00.md).
...
- Local accepted retired managed UI prototype experiment baseline: corrected retired prototype milestone through main
...
- retired managed UI prototype is the leading replacement candidate for the custom native UI stack.
  Evaluation happens only under `the removed prototype subtree`. No
  production renderer migration, GBSS removal, or widget cutover is implied
  until the user accepts retained retired prototype milestone evidence and a migration architecture.
- The candidate production presentation stack is intentionally bounded:
  .NET 10/C#, stable retired managed UI prototype Desktop 12 with its Skia renderer, AXAML and
  retired managed UI prototype styles/control themes, compiled XAML bindings, CommunityToolkit.Mvvm
  for shell/page state and commands, direct Microsoft dependency injection and
  logging abstractions only where lifecycle ownership benefits, retired managed UI prototype
  ...
  EF Core, retired third-party theme framework, SukiUI, or another navigation/theme framework by
...
- Testing for the candidate stack uses the repository-required MSTest.Sdk
  ...
  retired managed UI prototype.Headless/Skia tests for layout, focus and rendered frames, and a
  ...
  narrow adapters. The retired prototype milestone XInput adapter is retired by migration; the existing
...
- Retain the presentation-neutral `WidgetBridge` backend and its authenticated
  ...
  with a typed managed presentation-session facade. retired managed UI prototype controls never
  parse bridge transport or connect directly to widget processes.
- retired prototype milestone uses one generic current-`WidgetProtocol` semantic-tree adapter. It
  must not rebuild Settings, Audio Mixer, Network, media, library, Spotify,
  YT Music, or Game Launcher as domain-specific retired managed UI prototype pages.
- The retired managed UI prototype architecture decision is now accepted. DLV-216 and DLV-217 may
...
[implementation-agent-goal.md](../../../history/delivery-plan/implementation-agent-goal.md).
...
## retired managed UI prototype prototype lane

Task: Implementation agent — retired managed UI prototype prototype lane

Branch: retired prototype branch

### Recently completed — retired prototype milestone: transparent retired managed UI prototype overlay shell
...
retired prototype milestone therefore establishes visual feasibility, one stable shell/tray, four
representative retired managed UI prototype pages, standard controls/UIA, and an honest initial
...
into retired prototype milestone rather than blocking the next experiment.

### Recently completed — retired prototype milestone: controller-first retired managed UI prototype interaction
...
`the removed prototype subtree`, not a production migration. The user
...
The implementation uses retired managed UI prototype `FocusManager.FindNextElement`/`XYFocus` with
...
quality, and smoothness. retired prototype milestone therefore closes the isolated visual,
controller-routing, and spatial-navigation feasibility gates. This verdict
authorizes retired prototype milestone only; it is not a production migration decision.
...
licensing, package size, and compatibility with .NET 10/retired managed UI prototype. Prefer a
...
retired managed UI prototype controls through a parallel identifier registry.

Also correct the interaction and evidence gaps carried from retired prototype milestone:

- Remove the prototype's visual-tree-order `FocusNavigator` algorithm. Use
  retired managed UI prototype 12 `XYFocus`/`FocusManager` spatial navigation as the default target
...
- Controller A invokes the focused standard retired managed UI prototype control exactly once.
...
- Disconnect, reconnect, device loss, hidden lifecycle, focus loss, and route
  ...
  double-dispatch.
- Replace the tautological scale assertion with actual retired managed UI prototype render-scale
...
- Sample the retired managed UI prototype transition surface at start, midpoint, and completion so
...
4. Updated responsive tests exercise real retired managed UI prototype render scaling at the three
...
6. Standard retired managed UI prototype UIA remains the only accessibility tree. Controller input
   changes focus and invokes the same controls visible to UIA.
7. Run only the focused bounded retired prototype milestone suite and one exact-commit ordinary Windows
...
representative pages, publication, credentials, and an retired managed UI prototype migration
decision.
...
The historical retired prototype milestone stop condition was satisfied by the user's live
controller and visual verdict. Its production-host, privilege, credential, and
undocumented-window-manipulation prohibitions remain in force for later retired prototype milestone
experiments.

### Recently completed — retired prototype milestone: production-stack architecture slice
...
tuples, and schedules bound publication explicitly onto the retired managed UI prototype UI thread.
...
Dependencies: retired prototype milestone and retired prototype milestone accepted. Concurrency: isolated experiment
...
retired prototype milestone automatically.
...
- keep the remote contract UI-framework-neutral while a presentation-owned
  scheduler explicitly marshals bound view-model mutation to retired managed UI prototype's UI
...
Keep the current visual language on retired managed UI prototype styles/control themes over the
built-in Fluent control base; do not adopt retired third-party theme framework, SukiUI, ReactiveUI,
DynamicData, Serilog, EF Core, or another navigation/theme framework.

Replace the prototype transition with a native retired managed UI prototype page/composition
...
ordinary retired managed UI prototype virtualization. Prove bounded realized-container count,
...
typed retired managed UI prototype view models. Neither that contract nor the fake widget may
reference retired managed UI prototype, controls, view models, XAML, or UI-thread types. Prove a
...
2. View models own immutable presentation state and commands only. Focus,
   ...
   retired managed UI prototype presentation concerns with the accepted retired prototype milestone behavior intact.
...
5. Standard retired managed UI prototype UIA remains the only accessibility tree. Keyboard and the
...
system polish, retired prototype milestone, or a final migration decision. Stop if the architecture
requires retired managed UI prototype types in the public widget contract, a second focus/UIA tree,
production edits, privileged installation, credentials, or a material resource
increase with no clear owner.

### retired managed UI prototype Ready queue

### Current assignment — retired prototype milestone: complete polished overlay prototype

State: Assigned as a reuse-first migration program. Reconciliation is complete:
the standing retired managed UI prototype task is clean at accepted retired prototype milestone commit `2a3c722` and
...
Architecture contract: retired managed UI prototype presentation migration
plan.

Production code baseline: accepted local main `4f502c4`; accepted retired managed UI prototype
...
is part of retired prototype milestone.
...
- replace the presentation boundary: native declarative renderer/compositor,
  ...
  accessibility projection move behind retired managed UI prototype;
- implement one generic adapter from the current semantic widget presentation
  and action/focus identities into standard retired managed UI prototype controls. Do not rewrite
  eight domain pages or fork widget business logic merely to obtain retired managed UI prototype
  visuals; and
...
#### retired prototype milestone — managed presentation-session extraction

Owner: temporary `retired-session` task on `retired extraction branch`, created
...
managed bridge/runtime code and add focused tests. It does not edit retired managed UI prototype
...
transport. No retired managed UI prototype type crosses this facade. Commit once and stop.

#### retired prototype milestone — native platform interop extraction

Owner: temporary `retired-platform` task on `retired extraction branch`, created
...
and add focused parity tests. It does not edit WidgetBridge/runtime, retired managed UI prototype
...
placement, and focus/visibility events. The retired managed UI prototype shell owns the only
...
#### retired prototype milestone — generic retired managed UI prototype migration candidate

Owner: standing `retired-prototype` task on `retired prototype branch` after
...
Objective: replace the retired prototype milestone fake remote contract and fake widget pages with
...
generic advanced-presentation slot. Use standard retired managed UI prototype controls/UIA,
...
1. One simple widget, one provider-backed widget, one 10,000-item/full-
   ...
   retired prototype milestone evidence covers every installed tray widget through the same adapter.
...
4. Compact 420x340 and 978x466, standard, wide, and 100/125/150-percent render
   ...
   retired managed UI prototype UIA is the only accessibility tree.
5. The accepted retired prototype milestone motion/design quality remains: stationary tray, native
...
6. Verification is proportional: focused suites per extraction, one final
   bounded retired prototype milestone Release suite, one ordinary Windows bridge/runtime lifecycle,
   and one exact-final-commit measurement. No product aggregate merely for retired prototype milestone.
...
production cutover. retired prototype milestone and native-renderer/GBSS deletion remain out of
scope.

#### Superseded retired prototype milestone page inventory — not implementation authority
...
adapter above; do not execute any `retired prototype milestone`, `MEDIA`, or `LIBRARY` task.

Objective: turn the isolated retired managed UI prototype experiment into a cohesive, polished,
...
#### retired prototype milestone — integration lead

Branch/task: `retired prototype branch`, standing retired managed UI prototype prototype task.
Baseline: accepted corrected retired prototype milestone commit. Concurrency: first, before the page
tasks are created.
...
- one responsive shell, stationary tray, page host, modal layer, notification
  ...
  native retired managed UI prototype transition owner;
- coherent retired managed UI prototype tokens/control themes for typography, spacing, shape,
...
#### retired prototype milestone — system controls page lane

Temporary branch/task: `retired extraction branch`, `retired prototype milestone — system pages`.
...
#### retired prototype milestone — media page lane

Temporary branch/task: `retired extraction branch`, `retired prototype milestone — media pages`.
...
#### retired prototype milestone — library page lane

Temporary branch/task: `retired extraction branch`, `retired prototype milestone — library pages`.
...
Preserve retired prototype milestone item-stable identity and snapshot-owned exact action authority.
...
- native start/mid/end page transitions with a stationary tray, no deliberate
  ...
  standard retired managed UI prototype UIA only; and
...
3. Compiled bindings and typed templates remain enabled. Remote/domain fixtures
   ...
   retired managed UI prototype UI thread; exact snapshot item/action authority is preserved.
4. Standard retired managed UI prototype controls/UIA remain the only accessibility tree. Names,
...
5. One focused suite per work package is run once at its coherent boundary.
   ...
   retired prototype milestone MSTest.Sdk 4.3.2 suite, fixing only actual retired prototype milestone regressions; no product
...
7. The planner launches the copied exact retired prototype milestone runtime and leaves it running
...
cross-platform work, or retired prototype milestone. Stop for a required production edit,
...
State: Assigned. The accepted retired managed UI prototype architecture retains package-owned
...
runtime. Do not edit the retired managed UI prototype experiment or renderer while migrating the
...
4. **Held until the retired managed UI prototype decision and accepted DLV-216/DLV-217 — DLV-218: remove retired
