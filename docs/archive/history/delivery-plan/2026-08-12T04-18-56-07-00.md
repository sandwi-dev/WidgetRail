...
> Historical snapshot captured at 2026-08-12T04:18:56-07:00 after the active
> file crossed 1,000 physical lines. This preserves evidence only; it is not
> implementation authority. Return to [`../../delivery-plan.md`](../../delivery-plan.md)
...
[`2026-08-11T20-01-54-07-00.md`](2026-08-11T20-01-54-07-00.md).
...
- Tasks update directly affected public documentation and
  `docs/archive/implementation-status.md`; they never edit reviewer-owned files.
...
DLV-134's managed boundary is committed and held while platform DLV-145 adopts
it in the production host. DLV-135 is now the current widgets assignment and
may proceed concurrently because DLV-145 keeps one lane on the visible product
outcome. GOG and Amazon adapters remain planned until separately evidenced
bounded local installed-record and launch-revalidation contracts exist.
...
### Widgets held visible dependency

### DLV-134 — Project Game Launcher state into all four native experiences

**State:** Managed boundary committed as `e847502`, reviewed as coherent, and
held unintegrated with the complete DLV-134 outcome. It adds persisted
Hero Rail/Cover Wall/Carousel/Compact Grid selection and one pure six-slot
projection while preserving exact action, SavedId, focus, cursor, and provider
authority. Game Launcher passes 68/68. Ordinary OverlayHost does not yet adopt
the private marker/slot seam, so the visible experience does not change until
platform DLV-145 is accepted; do not integrate this commit alone.
...
DLV-135 is the current widgets assignment. After committing it, do not consume
another launcher milestone until DLV-145 and the held DLV-134 pair are reviewed
...
### DLV-145 — Adopt Game Launcher semantic slots in the production host

**State:** Assigned. Start from clean accepted main `73e1e17`, then consume held
managed dependency `e847502` on the platform branch only. Neither commit is
integrable as a visible DLV-134 outcome until this native hook is accepted.
...
documentation
**Concurrency:** widgets lane owns DLV-135 CLI/catalog tooling. No managed Game
...
Grid in Game Launcher changes the live native presentation while the same exact
games, actions, focus identities, cursor position, source truth, and Back route
remain authoritative and usable.

**Objective/scope:** recognize only the first-party
`game-launcher-experience` root plus exactly one each of the six documented
slot classes emitted by `e847502`; map the closed selected-profile class to the
existing native preset and route the already-admitted snapshot, current focus,
responsive viewport, accessibility/render options, and work area through
`LauncherExperienceAdapter::RenderExperience`. Use the adapter's canonical
semantic snapshot and render geometry for paint, pointer, focus navigation,
scroll reveal, action dispatch, and UIA publication. Missing, duplicate,
malformed, unknown-profile, or incompatible projections atomically use the
ordinary declarative path with one bounded diagnostic. Other widgets never
enter this path. Preserve instance/sequence/input scope, collection anchor and
keys, exact node/action/focus IDs, and the current last-good presentation.

**Acceptance:** all four profiles render through the ordinary production host
at compact/standard/wide, 125% DPI, 150% interface/text scale, taskbar-reserved
and non-primary work areas; each keeps exact A/View/X/Y/LB/RB/Back dispatch,
nearest valid focus across profile changes, collection continuation, pointer/
paint/focus/UIA agreement, long-title/missing-art fallbacks, and bounded current
...
usable ordinary declarative Game Launcher content; non-launcher widgets are
byte/semantic-equivalent to their prior path.
...
Tier 1 native launcher adapter/layout/renderer/focus/pointer/UIA Release groups;
Tier 2 one fresh production-host fixture using the held installed Game Launcher
package plus the 68/68 managed result as retained dependency evidence. No
...
Return the exact mismatch for planner serialization instead of weakening the
fallback or editing managed code.
...
2. DLV-131/132/133/137/140/141/143 are accepted. DLV-145 is the active platform
   assignment and consumes held managed DLV-134 boundary `e847502` without
   making that partial control independently integrable.
3. DLV-135 is the active widgets assignment. Integrate the held DLV-134/DLV-145
   pair before claiming live production projection; DLV-135 may integrate
   afterward or independently only if its authoring artifacts do not expose a
   preview claim that depends on the unaccepted hook.
