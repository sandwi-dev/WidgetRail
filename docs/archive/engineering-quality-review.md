...
Last targeted reassessment: 2026-08-16 for EQ-003 against planner `main`
`44f0c33`; unaffected findings retain their recorded baselines<br>
...
| EQ-003 | P1 | **Decomposition declined; condition accepted.** `OverlayApp` is the dominant native mutable owner at 13,432 class lines, 173 methods, and 148 fields — grown from 8,749/138 at the DLV-255 baseline. DLV-273 attempted the widget-content-presenter seam, was physically rejected, and was retired at `2d037171`; it established that `inputOwner`, `renderedFocusId`, `semanticFocus`, presentation authority, and `lastWidgetPresentationPaintKey_` are one atomic frame commit. `docs/archive/overlay-host-main-refactoring-review.md` records the declined decision, that invariant, and the rejected seams. The class is a coordinator arbitrating ~28 focused owners, so a further extraction adds an arbitration edge rather than removing coupling. Located code findings are tracked separately in `docs/overlay-host-code-findings.md`. | No extraction is assigned. Contain the condition instead: a build-failing ceiling on `OverlayApp` field count, and the OH-001/OH-002 findings addressed in place. Reopen only with a newly approved seam that leaves committed checkpoint and interaction publication in the accepted owner and begins with deterministic Back/focus regression proof. |
...
| EQ-020 | P1 | **Closed through accepted and integrated DLV-231/234/235 at main `5440e7b`.** DLV-231 keeps delayed/nonresponsive work generation-bound and revocable without blocking tray/controller/close routing. DLV-234 closes partial-frame desynchronization through bounded taint-and-replace ownership of the sole bridge transport. DLV-235 retains the typed actionable worker-start failure through lifecycle retarget/revocation until explicit Retry admits a valid fresh snapshot. The user-authorized clean-main reconstruction preserved stable product/test patch identity, focused evidence is green, and the coherent native Release was rebuilt and visibly launched. | Reopen only for a reproduced stale admission, UI stall, framing loss, lost primary failure, or abnormal worker/process cleanup. |
...
| --- | ---: | --- |
| `OverlayApp` / `main.cpp` host application | 8,749-line class; 307 members; 138 fields on DLV-255 baseline | **DLV-255 analysis complete; extraction not yet assigned.** Follow `docs/archive/overlay-host-main-refactoring-review.md`: diagnostics, accessibility publication, interaction state, fixed chrome after DLV-254, then widget painting. Keep state transition/session orchestration in the top-level host, require a before/after authority map per extraction, and reject cosmetic splitting or another coordinator/status cache. |
