...
- Exact-loopback authority fixes the destination to one manifest-declared
  ...
  companion HTTP and private secrets](../reference/community-companion-services.md).
- Private widget state stores exactly one canonical JSON document/tombstone per
  ...
  state](../reference/private-widget-state.md).
...
[widget capabilities](../reference/capabilities.md) for the sandboxed developer and
transport contract.
...
See [publishing and installation](../developers/publishing-and-installation.md) and the
detailed [package safety contract](../reference/widget-packaging.md).
