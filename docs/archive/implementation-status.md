...
overlay. See [Games & Apps](../reference/games-and-apps.md).
...
- Spotify now has a typed v1 broker contract, a composed trusted provider,
  ...
  integration status](../reference/spotify-integration.md).
...
- Controller Settings, strict persistence, version-pinned theme selection,
  ...
  and global themes](../reference/settings-and-themes.md) and [theme packaging and
  distribution](../developers/theme-packaging.md).
- Audio Mixer is a bounded first-party public-SDK reference slice: its widget,
  ...
  [widget capabilities](../reference/capabilities.md) and [Windows provider
  architecture](../maintainers/windows-provider-architecture.md) and [Network Controls
  reference](../reference/network-controls.md).
...
See the [documentation index](../README.md), [security and trust](../maintainers/security-and-trust.md),
and [troubleshooting](../users/troubleshooting.md).
...
The compatible runtime-v2 envelope serializes render publication through the
existing widget owner, and the Bridge reuses its existing exact-generation
retained snapshot. Existing installed full-trust applications can continue to
send v2 hello/checkpoint traffic; `{}` render requests remain complete
checkpoints, while optional render fields negotiate atomic updates only between
current peers.
...
limits, an intermediate tree that exceeds the final depth/node limits, and a
cheaper checkpoint all fall back or fail closed as appropriate.
...
the frozen-runtime-v2 handshake/checkpoint and current-peer negotiated-update
Runtime cases 1/1 each, retained pre-correction WidgetRuntime 76/76, WidgetSdk
compatibility 12/12, and the two directly
...
MSBuild writes. One correction full-Runtime attempt stalled at its bound after
six green direct ownership cases and is not counted as suite evidence. The
restricted runner separately denied the exact local pipe/process fixtures;
both directly affected cases pass outside that restriction.
Documentation validation remains red only on 13 links in
