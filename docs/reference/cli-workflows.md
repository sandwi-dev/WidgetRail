...
- .NET SDK 10.0.302 to build this repository, plus the .NET 8 runtime
- .NET 8 SDK or later for a scaffold outside this repository
...
The current local pre-release unit is `0.3.0-dev`. The generated package adds a
...
see [Widget SDK compatibility and release unit](../developers/widget-sdk-compatibility.md)
...
[embedded-media template guide](../developers/embedded-media-widget.md) explains runtime
...
[Activity provider contracts](../maintainers/windows-provider-architecture.md) shows a read-only event stream with
...
[widget authoring guide](sdk-reference.md#validate-list-scenarios-render-replay-and-test)
...
New widget IDs are disabled. Open the overlay's Settings → Widgets
...
service, open that package under Settings → Widgets and use
...
**Settings → Widgets**, select the exact built-in or Community widget,
...
To remove a disabled Community package, return to its Widgets details
...
active version, disable the widget. In Settings → Widgets, open the
...
**Settings → Widgets → Catalog recovery**, choose an inactive,
...
If the quota appears while **Widgets** already has a validated
...
installation](../developers/publishing-and-installation.md). Theme authors should instead use
the dedicated [theme packaging and distribution](../developers/theme-packaging.md) workflow.
