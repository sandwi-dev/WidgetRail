# SDK reference and advanced examples

Start with the [authoring guide](../developers/widget-authoring-guide.md) for a learning path. This page covers the detailed APIs and runtime rules. The [protocol constants](../../src/WidgetProtocol/ProtocolConstants.cs) define current feature versions.
...
Ordinary declarative controls use the native renderer. Embedded web media is a separate host-managed feature. Do not design around HTML, DOM APIs,
...
Run the [canonical offline author journey](cli-workflows.md#create-and-build-a-widget)
...
[Widget SDK compatibility workflow](../developers/widget-sdk-compatibility.md); additions,
...
the [embedded-media lifecycle cookbook](../developers/embedded-media-widget.md). It stages the
...
[quickstart](cli-workflows.md#canonical-generated-widget-source). The
...
and entering-edge focus. For viewport-driven collections, use the implemented
`WidgetCursorResource<TItem>` with the shared collection metadata; see
[Data and lifecycle](../developers/data-and-lifecycle.md). When using `Paginate` directly, keep only a small current page
...
[Immutable widget models](widget-model-reference.md).
...
1. Open **Widgets** and review identity, version, host range,
...
revocation, automatic update discovery, and a public gallery are not implemented.
Settings does support local package installation, update review, and removal.
Users should still review the source and publisher before enabling a package.
...
Read [performance evidence](../maintainers/performance.md) before claiming latency, CPU, GPU,
or memory results.
...
Before filing or closing a bug, check the [Compatibility notes](../users/known-limitations.md)
...
- automatic update discovery or a built-in online widget marketplace;
...
- arbitrary browser DOM, browser CSS, shaders, or custom native drawing for
  ordinary declarative controls (supported SVG icons and embedded media have
  separate contracts);
- arbitrary raw HID access from sandboxed widget code;
- general CPU/disk quotas (memory/process containment and lifecycle policies
  have their own supported contracts); or
- universal Fullscreen Exclusive, anti-cheat, Guide-device, input-suppression,
  mixed-monitor, or game compatibility.

## Reference map

- [Widget quickstart](cli-workflows.md)
...
- [Publishing and installation](../developers/publishing-and-installation.md)
