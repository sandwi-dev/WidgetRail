...
| GBA-003 | P1 | Confirmed regression; DLV-121 queued after DLV-118 | Audio Mixer / declarative renderer / host UIA | Accepted DLV-049 established the four-session Microphone-to-Master edge, but the accepted-main Release build on `048e0a3` failed `AudioMixerScrollHostTests` with `Requested production UIA focus target was absent.` The unchanged failure was not rerun. DLV-121 must trace snapshot, scope, rendered bounds, and UIA projection before choosing the owning lane; fresh live keyboard/controller confirmation also remains. |
