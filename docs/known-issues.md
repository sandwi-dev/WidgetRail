...
| GBA-045 | P0 | Verifying | Spotify widget / Widget SDK / protocol v11 | The playlist bridge overflow is fixed by automatic focus-edge pagination: playlist and track lists render one 12-row page, cache at most six pages, and expose no Load-more row; focused SDK/widget tests cover forward, reverse, and cached navigation, while packaged controller evidence remains. |
...
| GBA-047 | P1 | Verifying | Widget SDK operations / Spotify Community addon | Public bounded SingleFlight, Latest, and Serial operation lanes now bind explicitly to Active, State, or Widget lifetimes and drain before lifecycle callbacks. Spotify 0.2.8 migrated automatic destination/playlist paging to one Active Latest lane, removing its page task, cancellation source, and generation counter; packaged lifecycle/controller evidence and broader widget migrations remain. |
