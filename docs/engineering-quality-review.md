...
1. **Finish platform DLV-021, then correct GBA-003 through DLV-049.** DLV-026 is
   integrated as `4957101` but failed the user's live keyboard and controller
   check. Finish the already-active geometry milestone, then prove the emitted
   Microphone Up edge and correct the retained scroll/extent/reveal state before
   DLV-015. In parallel, DLV-021 must correct
