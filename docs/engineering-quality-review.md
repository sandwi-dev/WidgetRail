...
| EQ-003 | P1 | **Analysis completed by DLV-255; implementation deferred behind active visible work.** `OverlayApp` remains the dominant native mutable owner at 8,749 class lines, 307 members, and 138 fields. `docs/overlay-host-main-refactoring-review.md` maps the authorities and prioritizes diagnostics, accessibility publication, interaction state, fixed chrome, and widget painting as serialized extractions. Another session coordinator, resource service locator, message-switch-only move, partial/include split, or new HWND/focus/transport authority is rejected. | Separate assigned milestones materially move each named state/invariant cluster behind typed inputs/outcomes, reduce `OverlayApp` field and responsibility ownership, preserve generation/lifetime authority, and provide the focused proof named in the DLV-255 review without recreating equivalent fan-out elsewhere. |
