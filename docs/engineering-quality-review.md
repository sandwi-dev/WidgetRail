...
| Bridge client lifecycle | DLV-039 candidate `57befdd` and corrections `57f3e951` and `e1df09c` are not yet accepted. The latest correction reduces the server to 904 lines and places catalog, current-generation, residency, idle-unload, restart, notification admission, and terminal disposal behind a 1,264-line registry plus a 199-line bounded notification lane. It closes the previously reviewed generation, publication, cancellation, lock-order, resource-bound, and fatal-outcome defects; scoped evidence passes Runtime 74/74, Bridge 64/64, and docs 52. The new registry remains an architecture-review hotspot, currently dispositioned by the open DLV-039 acceptance correction rather than treated as closed | Require one direct queued-versus-in-flight event cancellation/frame-integrity fixture, accurate documentation of the limited internal pipe-session behavior change, and removal or real consumption of the unused stored retirement task. On acceptance, record a conditional cohesive exception only if the registry remains the singular worker-generation state/transition owner and reopen it if diagnostics, pipe/session/write, dispatcher, or another independent coordination policy enters the type. DLV-040 remains the separate diagnostics/recovery extraction. |
...
2. **Widgets DLV-039 — extract bridge client and residency ownership.**
   ...
   Candidate `57befdd` achieves the intended ownership reduction and correction
   `57f3e951` closes its original restart-generation, publication TOCTOU, and
   multi-client disposal gaps. Correction `e1df09c` then closes the reviewed
   unbounded notification/failure retention, restart-cancellation, external-
   disposal lock-order, and fatal-outcome defects; scoped evidence passes
   Runtime 74/74, Bridge 64/64, and docs 52. It remains unaccepted only for a
   narrow final correction: directly prove queued cancellation versus in-flight
   frame deadline/session termination, accurately document that internal pipe-
   session behavior changed without changing the public protocol or framing
   format, and remove or genuinely consume the otherwise dead stored retirement
   task. Runtime is unchanged and must not be rerun.
...
4. **Continue responsibility milestones rather than line-count refactors.**
   ...
   accepted DLV-037 the managed worker client. DLV-039 and DLV-040 then separate residual bridge
