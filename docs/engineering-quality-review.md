...
Date: 2026-08-10<br>
Last reassessed: 2026-08-10 against integrated `main` `5aedfe8`, including
accepted DLV-004, DLV-009, DLV-014, and DLV-017 implementation/evidence plus
...
widget lifecycle migration, and production-host action-failure composition are
accepted. Trusted artwork, shared list/focus and geometry corrections, Audio
...
| Responsive/controller UI | DLV-003 (`27b0319`, integrated by `703c5bb`) owns the narrow Button placement slice. DLV-005 (`3fc3770`, integrated by `aaf36d9`) adds one host-owned 700 ms tray-Y recognizer. DLV-020 (`b0c95ca`, integrated by `7cda335`) retains one admitted visual-only surface through destination startup, but its static production-HWND evidence did not predict real temporal behavior. The 2026-08-10 Games & Apps user run shows missed frames, whole-interface flicker, and exposed gray/black bands during live extent animation. The 16 ms timer, per-frame placement, `WM_SIZE`/Direct2D resize, and forced redraw all execute on the native UI thread; synchronous bridge calls are an additional measured-risk boundary, not yet the proven cause. Remaining evidence also shows Spotify list-boundary jumps, header/list oscillation, clipped SectionHeader text, and shared Now Playing alignment imbalance | DLV-025 preempts DLV-021 and must prove atomic real-product presentation plus frame cadence, falling back from live HWND resizing if necessary. DLV-021 then owns shared component geometry; DLV-006 plus DLV-022 own continuous list/focus composition. |
| Games & Apps | DLV-004 (`7e0b83e`, integrated by `76032bb`) accepts the shared responsive surface and bounded Catalog. DLV-017 (`24a8944` plus `b844fd8`, integrated by `5aedfe8`) adds a bounded display-only warm start and exact current-lifetime AppId admission with focused SDK 84/84, worker 9/9, and Games 49/49. User testing then exposed an untested catalog-remove/Back sequence that can hide unrelated Library rows, plus unstable Add applications visibility and low 430-DIP density | DLV-024 must correct and prove managed mutation/projection plus last-good presentation continuity before GBA-063 closes. DLV-018 owns trusted lazy artwork after DLV-006; retain packaged and physical evidence |
...
| Visual evidence | Provenance-aware offscreen widget-body capture exists, but its dirty old Spotify 0.1.6 setup matrix neither covers current advanced states nor judges layout/visual correctness. DLV-020 adds 44 reviewed real-HWND frames that removed the startup surface in its bounded harness, but user evidence now disproves temporal product closure: the real Games & Apps resize tears and stutters between captured endpoints. Other captures still show clipped Spotify `LIBRARY`, missing Games & Apps artwork, and shared Now Playing alignment imbalance | DLV-025 requires timestamped real-first-party transition intervals and frame-time evidence, not solid-color synthetic endpoints alone. Then retain the clean current package/state/profile and physical full-shell/controller/DPI matrix. |
...
1. **Platform DLV-025 — correct transition tearing and cadence.** Reproduce the
   real Games & Apps switch, measure the single UI-thread placement/resize/
   redraw path and any overlapping synchronous bridge work, and retain temporal
   first-party evidence. Prefer an immediate stable switch over laggy motion.
2. **Widgets DLV-024 — stabilize Games & Apps Library continuity.** Reproduce
   remove-one/Back exactly, preserve every unrelated durable and visible row,
   keep Add applications stable, and use more safe vertical space.
3. **Platform DLV-021 — close shared component geometry.** Correct shared
   ...
   imbalance.
4. **Then recover Spotify without replacing last-good UI.** Widgets DLV-023
   follows DLV-024 with typed transient/fatal recovery and bounded backoff.
