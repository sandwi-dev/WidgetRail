...
| EQ-020 | P1 | **DLV-231 source `2a379ac` remains rejected pending DLV-235; DLV-233 `1323c8a` and DLV-234 `9435050` are accepted but unintegrated.** DLV-234 closes the partial-frame desynchronization risk through taint-and-replace ownership of the sole bridge transport with direct empty/header/body-prefix cancellation recovery and bounded process cleanup. The cumulative action-failure route still reaches the product and then loses the primary worker-start failure before Retry. | Retained primary failure precedence through lifecycle/revocation, explicit Retry recovery, preserved DLV-234 frame-safe replacement, existing production-host timing/stale-publication evidence, and bounded sole-transport/process teardown. |
...
| --- | ---: | --- |
| `OverlayApp` / `main.cpp` host application | `main.cpp` remains the dominant several-thousand-line logical owner | **Managed by DLV-235 after accepted DLV-234 source above rejected DLV-231.** Slow-worker responsiveness and frame-safe cancellation recovery now stay in the existing session/bridge owners; primary failure precedence must remain in the existing failure owner. Any material `main.cpp` change requires a before/after responsibility map and may not add another coordinator or status cache. Revisit further extraction only as a named visible/release prerequisite. |
