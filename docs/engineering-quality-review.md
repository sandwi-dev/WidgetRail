...
| EQ-020 | P1 | **DLV-231 source `2a379ac` rejected pending DLV-234 then DLV-235; DLV-233 source `1323c8a` is accepted but unintegrated.** Slow-worker responsiveness and stale-generation evidence are green, but cancelling a synchronous partial frame can desynchronize the reused bridge byte stream. The cumulative action-failure route also reaches the product and then loses the primary worker-start failure before Retry. | Frame-safe cancellation recovery, retained primary failure precedence through lifecycle/revocation, explicit Retry recovery, existing production-host timing/stale-publication evidence, and bounded sole-transport/process teardown. |
...
| --- | ---: | --- |
| `OverlayApp` / `main.cpp` host application | `main.cpp` remains the dominant several-thousand-line logical owner | **Managed by DLV-234 and DLV-235 corrections after rejected DLV-231 source.** Slow-worker responsiveness, frame-safe cancellation recovery, and primary failure precedence must stay in existing session/bridge/failure owners; any material `main.cpp` change requires a before/after responsibility map and may not add another coordinator or status cache. Revisit further extraction only as a named visible/release prerequisite. |
