...
| Responsive/controller UI | DLV-003 (`27b0319`, integrated by `703c5bb`) owns the narrow Button placement slice. DLV-005 (`3fc3770`, integrated by `aaf36d9`) adds one host-owned 700 ms tray-Y recognizer. DLV-020 (`b0c95ca`, integrated by `7cda335`) retains one admitted visual-only surface through destination startup, transfers semantic authority immediately, and retargets a bounded in-place extent transition from presented geometry. Focused Release evidence and 44 reviewed production-HWND frames cover delayed Spotify/Games startup, reversal, and reload without the startup dialog, black clear, square edge, stale extent, or tray loss. Remaining evidence still shows Spotify list-boundary jumps, header/list oscillation, clipped SectionHeader text, and shared Now Playing alignment imbalance | DLV-021 owns shared component geometry; DLV-006 plus DLV-022 own continuous list/focus composition. Retain physical controller/full-shell/display evidence at 100-150% scale |
...
| Visual evidence | Provenance-aware offscreen widget-body capture exists, but its dirty old Spotify 0.1.6 setup matrix neither covers current advanced states nor judges layout/visual correctness. DLV-020 adds 44 reviewed real-HWND transition frames that settle the startup-dialog/black-clear switch defect at the automated level. User-supplied captures still concretely show clipped Spotify `LIBRARY`, missing Games & Apps artwork, and shared Now Playing alignment imbalance | Clean current package/state/profile matrix covering the remaining reported surfaces, semantic icon-label/header/clipping assertions, reviewed tolerant baselines, and physical full-shell/controller/DPI smoke |
...
1. **Widgets DLV-024 — stabilize Games & Apps Library continuity.** Reproduce
...
2. **Platform DLV-021 — close shared component geometry.** Correct shared
   Button/ActionSurface/SectionHeader measurement and paint after accepted
   DLV-020, including the reported clipped Spotify header and cross-widget
   alignment imbalance.
3. **Then recover Spotify without replacing last-good UI.** Widgets DLV-023
   follows DLV-024 with typed transient/fatal recovery and bounded backoff.
4. **Keep shared foundations serialized.** DLV-006 remains the named continuous
