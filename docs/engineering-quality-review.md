...
| EQ-020 | P1 | **Accepted cumulatively through DLV-235 `b0ea2b4`, pending the implementation-owned status append merge and planner integration.** DLV-234 closes partial-frame desynchronization through bounded taint-and-replace ownership of the sole bridge transport. DLV-235 carries the existing typed worker-start category through the private adapter and retains its actionable failure across lifecycle retarget/revocation until explicit Retry admits a valid fresh snapshot. | Integrate the accepted DLV-231/233/234/235 chain, rebuild the coherent main Release, and retain live user evidence for responsiveness/recovery; reopen only for a reproduced stale admission, UI stall, framing loss, or lost primary failure. |
