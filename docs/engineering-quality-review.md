...
| EQ-020 | P1 | **Accepted cumulatively through DLV-235 `b0ea2b4`, but integration is blocked by a preserved substantial merge conflict against main `d4766e8`.** DLV-234 closes partial-frame desynchronization through bounded taint-and-replace ownership of the sole bridge transport. DLV-235 retains the typed actionable worker-start failure through lifecycle retarget/revocation until explicit Retry admits a valid fresh snapshot. | Obtain user recovery direction, reconstruct the accepted prefix from clean main under the platform lane's ownership, independently review any product/build conflict resolution, then rebuild the coherent Release. Reopen behavior only for a reproduced stale admission, UI stall, framing loss, or lost primary failure. |
