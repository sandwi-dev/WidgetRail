...
| Native host ownership | Proven low-level input, focus, lifecycle, bridge, renderer, singleton-process, and pinned-surface helpers. Accepted DLV-058 (`e160690`, integrated through `ae34f9a`) established the singular surface owner; accepted DLV-068 (`b83b3f7`, integrated through `9e795ac`) adds a separate 421-line pure placement/store policy while the `WidgetSurfaceCoordinator` aggregate grows from 653 to 964 physical lines to compose its HWND, gesture, renderer, and UIA routes. Rejected DLV-069 candidate `ea2691c` adds controller focus, pointer/UIA action composition, a bounded input queue, feedback, and emergency hide, growing the coordinator to 1,416 physical lines; its pure `ResolveControllerCommand` seam is narrow, but Click-through then covers the admitted content with an opaque placeholder. Accepted DLV-070 (`c61a49d`, integrated through `0b21384`) isolates process election/activation before platform initialization. `OverlayApp` is now about 5,500 physical lines and remains the mutable cross-subsystem hotspot. | **Explicit dispositions:** DLV-073 is the active visible correction and hotspot boundary for the unintegrated DLV-069 candidate. It must keep widget rendering visible while making Click-through interaction/UIA inert, report the coordinator's final responsibility/coordination map, add no second focus/lifecycle/renderer owner, and either justify a tightly named conditional cohesive exception or expose one small pure mode-policy seam without broad refactoring. Do not integrate DLV-069 or begin DLV-062 until that correction is accepted. DLV-033 remains dependency-blocked for `OverlayApp` session-state extraction after DLV-025/DLV-032. Do not displace the visible correction merely to reduce line count. |
