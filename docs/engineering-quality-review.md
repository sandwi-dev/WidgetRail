...
| EQ-020 | P1 | **DLV-231 source `2a379ac` rejected pending DLV-234 after DLV-233.** Slow-worker responsiveness and stale-generation evidence are green, but cancelling a synchronous partial frame can desynchronize the reused bridge byte stream. | Frame-safe cancellation recovery plus the existing production-host timing/stale-publication evidence, with bounded sole-transport/process teardown. |
