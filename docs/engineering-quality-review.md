...
| Responsive/controller UI | DLV-003 (`27b0319`, integrated by `703c5bb`) owns the narrow Button placement slice. Accepted DLV-021 (`b714efe`, integrated by `bc2de86`) adds one DirectWrite measurement/paint plan, final-width row remeasurement, outward pixel rounding, and exact Games/Spotify/Now Playing/Settings/SDK Gallery component profiles; focused counts are 25 text-layout, 250 layout, 4,769 renderer, and 589 shared-geometry checks. DLV-005 (`3fc3770`, integrated by `aaf36d9`) adds one host-owned 700 ms tray-Y recognizer. DLV-026 (`979de24`, `68efc70`, `9cc633a`, integrated by `4957101`) adds a scale-aware one-native-pixel reveal tolerance while retaining wrong-axis, larger-overlap, and fixed-clip rejection; its screenshot-excluded production-HWND/UIA manifest records 84 preferred/constrained/150% traversal and churn states, but the user reproduced the reverse dead end with keyboard and controller in that freshly built Release. Cycling away/back repairs navigation after a small automatic upward movement, and the live log records `value_clamped [audio.root] scrollOffset ... outside its safe range`. DLV-020 (`b0c95ca`, integrated by `7cda335`) retains one admitted visual-only surface through destination startup, but its static production-HWND evidence did not predict real temporal behavior. DLV-025 then measured real populated first paints near 31 ms, six successful Spotify paints over 674 ms for 14 inputs, and five consecutive corrected captures with an exposed dark band. Removing repeated HWND interpolation was insufficient: resizing the current Direct2D HWND target exposes undefined content before successful draw, and a later `DwmFlush` cannot undo already composed frames | DLV-049 is active before DLV-015: the emitted Microphone Up edge is confirmed, so the platform lane is reproducing and correcting retained offset/extent/reveal reconciliation in the exact four-session product shape. DLV-025 remains blocked at its documented new-compositor stop condition; resume only after a bounded user-authorized offscreen atomic-present or DirectComposition/swap-chain design. DLV-006 plus DLV-022 own continuous list/focus composition. GBA-003 is Confirmed, not Verifying; GBA-058/GBA-060 are Verifying pending the fresh packaged visual check. |
...
1. **Correct GBA-003 through active DLV-049.** DLV-026 is integrated as
   `4957101` but failed the user's live keyboard and controller check. DLV-021 is
   now accepted and integrated as `bc2de86`; its shared text/component correction
   awaits the fresh packaged visual check. The emitted Microphone Up edge is
   confirmed, so DLV-049 must now correct the retained scroll/extent/reveal state
   in the exact four-session shape before DLV-015. This current visible defect
   outranks further behavior-preserving decomposition.
