...
2. **Widgets DLV-037 — split managed worker-session transport from gesture authority.**
   ...
   retaining the documented root exception. DLV-037 now owns the roughly
   1,027-line `WidgetProcessClient` boundary and must give process/session
   transport, pending requests, leases, and gesture reservations singular
   terminal owners without changing protocol or authority.
...
4. **Continue responsibility milestones rather than line-count refactors.**
   ...
   assigned DLV-037 the managed worker client. DLV-039 and DLV-040 then separate residual bridge
