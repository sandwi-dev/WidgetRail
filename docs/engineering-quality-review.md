...
| Native host ownership | Proven low-level input, focus, lifecycle, bridge, renderer, singleton-process, and pinned-surface helpers. Accepted DLV-058 (`e160690`, integrated through `ae34f9a`) established the singular surface owner; accepted DLV-068 (`b83b3f7`, integrated through `9e795ac`) adds a separate 421-line pure placement/store policy while the `WidgetSurfaceCoordinator` aggregate grows from 653 to 964 physical lines to compose its HWND, gesture, renderer, and UIA routes. Accepted DLV-070 (`c61a49d`, integrated through `0b21384`) isolates process election/activation before platform initialization, but `OverlayApp` remains the mutable cross-subsystem hotspot at 5,345 physical lines. | **Explicit dispositions:** DLV-069 is the active visible completion boundary for the existing surface owner and must report its final responsibility map; do not add a second focus/lifecycle authority. After DLV-069, treat the coordinator as a conditional cohesive exception only if placement remains in the pure policy and the coordinator retains one HWND-composition responsibility; otherwise queue a bounded extraction before further growth. DLV-033 remains dependency-blocked for `OverlayApp` session-state extraction after DLV-025/DLV-032. Do not displace current visible work merely to reduce line count. |
