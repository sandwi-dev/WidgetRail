...
| EQ-003 | P1 | Review Assigned as DLV-255. `OverlayApp` remains the dominant native mutable owner even after focused session, placement, process, surface, and launcher seams were extracted. DLV-255 produces a responsibility/authority map and prioritized behavior-preserving extraction plan before any refactor is authorized. | `docs/overlay-host-main-refactoring-review.md` identifies real ownership seams, dependencies, risk, and focused proof using exact-main clangd plus manual lifetime/caller review; cosmetic splitting is rejected. |
...
| --- | ---: | --- |
| `OverlayApp` / `main.cpp` host application | `main.cpp` remains the dominant several-thousand-line logical owner | **Managed through integrated DLV-235 at main `5440e7b`; DLV-232 is the assigned next platform milestone.** Slow-worker responsiveness, frame-safe cancellation, and primary failure precedence stay in the existing session/bridge/failure owners; DLV-235 adds only the private typed stage mapping at the adapter. Any material `main.cpp` growth still requires a before/after responsibility map and may not add another coordinator or status cache. |
