...
DLV-018 candidate `039b7b8` makes this open ownership gap directly relevant to
a visible feature. The image-cache worker now calls
`WidgetBridgeClient::ResolveArtwork` and holds a new recursive mutex that every
UI-thread bridge method also acquires. Moving provider rasterization off the UI
thread is therefore insufficient: while the worker waits for the broker's
bounded artwork result, event pumping, action/lifecycle requests, catalog work,
and `Stop` can wait on the same mutex. The candidate has no manually blocked
provider fixture proving concurrent UI/control-plane progress. DLV-054 owns
only this newly introduced artwork-demand contention and must not claim the
broader startup/ACL/session-coordinator problem in EQ-020 is closed.
...
| Spotify | DLV-007 (`ff706d2`) gives rendering one immutable keyed presentation revision. DLV-008 (`2f42ab8`, integrated by `80e54af`) preserves singular ownership while arranging lifecycle/action wiring, routes, playback behavior, and snapshot-only presentation into named partial files; the current aggregate remains one roughly 2,040-line class with unrestricted member access. Accepted DLV-023 (`3cfdd27`, integrated by `4dc1bd5`) adds a widget-private typed transient/fatal policy. Accepted DLV-021 corrects shared Library-header measurement. Accepted DLV-051 (`dc22202`, integrated by `822d29c`) authors the inactive seek Slider Left edge to the selected wide rail destination or compact Player tab. DLV-022 candidate `c349bbd` supplies the shared exact-intent join and continuous lists; accepted-at-code DLV-053 (`7f5c2fd`) adds one bounded private occurrence policy, exact stale-generation rejection, exact keyed playback lookup, and the singleton graph correction. Spotify 45/45, worker 6/6, docs 53, and package validation pass | Keep the DLV-022/053 product fix unintegrated until active DLV-054 corrects preceding DLV-018. Then launch the coherent Release for live list/header verification before DLV-043 replaces the aggregate partial owner with real boundaries. Keep GBA-056/GBA-058/GBA-061 Verifying until live recurrence testing; run live auth/playback gates only when authorized |
...
3. **Finish the current visible widgets sequence.** DLV-006 is accepted through
   `9c7438f`. DLV-053 commit `7f5c2fd` is accepted at code level and corrects
   DLV-022's duplicate-occurrence and singleton-row defects. DLV-018 candidate
   `039b7b8` remains unintegrated because slow artwork demand
   serializes UI bridge work and same-identity changed icons retain stale cache
   identity. Finish active DLV-054, integrate and launch the coherent visible
   prefix if accepted, then take DLV-043 architecture cleanup.
