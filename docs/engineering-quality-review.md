...
| `WidgetProcessClient` | 1,009 on accepted `main`; candidate `b6a4de3` is 833 | DLV-037 correction requested for construction/Stop terminal linearization and stale-effect publication. |
...
**Status: DLV-031, DLV-032, DLV-034, DLV-035, DLV-036, DLV-041, DLV-042, and
DLV-044 are accepted and integrated. Remaining concentration has bounded
delivery coverage in DLV-037, DLV-039, and DLV-040. Existing EQ-020 remains the
...
| Managed worker client | DLV-037 candidate `b6a4de3` reduces the roughly 1,027-line `WidgetProcessClient` to 833 lines and gives pending requests, gesture reservations, and terminal session resources named owners, with retained focused evidence passing Runtime 69/69, Bridge 52/52, worker 9/9, and docs 52. Review rejected the candidate because Stop can complete terminal cleanup while connection construction continues attaching process/job, transport, companion, leases, or tasks, and stale notification/process/gesture effects have only a pre-publication identity check rather than a linearized current-session admission boundary. The direct tests isolate dictionaries but do not force those actual client races | DLV-037 remains open for a bounded correction that linearizes construction/Stop/resource attachment and current-session effect publication, with manually controlled no-sleep Stop/connect, stale notification/process-exit, and gesture-replacement fixtures. Preserve one lifecycle/session owner and do not change protocol or threat model |
...
2. **Widgets DLV-037 — split managed worker-session transport from gesture authority.**
   DLV-044 is accepted as `8599694` and closes Settings partial ownership while
   retaining the documented root exception. Candidate `b6a4de3` establishes
   named process/session, pending-request, lease, and gesture owners, but review
   rejected its terminal and stale-publication ordering. Accept only after the
   bounded correction proves Stop versus construction, attachment after
   terminal, stale process/notification effects, and gesture replacement with
   deterministic manually controlled fixtures.
...
4. **Continue responsibility milestones rather than line-count refactors.**
   ...
   DLV-037's requested correction the managed worker client. DLV-039 and DLV-040 then separate residual bridge
