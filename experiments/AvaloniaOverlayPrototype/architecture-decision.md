...
| Outer geometry | Validated `WidgetSurfaceHints`, host work-area/DPI/accessibility constraints, native placement, and one Avalonia HWND | Admit each current view's preferred/minimum pair atomically, clamp it without a preset table, and size the HWND to the content-plus-chrome union around one bottom-center anchor. Content/HWND dimensions vary; tray and guide retain invariant absolute screen bounds. |
