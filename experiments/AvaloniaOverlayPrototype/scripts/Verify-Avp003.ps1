...
function Confirm-InvalidCompiledBindingFails {
    ...
    try {
        ...
        if ($output -notmatch 'AVLN2000' -or $output -notmatch 'PropertyThatDoesNotExist') {
            throw 'The invalid fixture failed for an unexpected reason instead of compile-time binding validation.'
        }
        Write-Host '[AVP-003 invalid binding fixture] expected AVLN2000 confirmed.'
    }
    finally { $process.Dispose() }
}

Invoke-BoundedDotnet -Label 'AVP-003 Release build' -Arguments @('build', $solution, '--configuration', 'Release', '--nologo')
