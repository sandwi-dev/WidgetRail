...
internal static class VersionCommand
{
    private const string HelpText = """
        ...
        not enable it; review the package, then run gbar enable explicitly.
        """;
        ...
        CancellationToken cancellationToken)
    {
        ...
        return args[0] switch
        {
            ...
            _ => throw new CliUsageException($"Unknown version command '{args[0]}'. Run 'gbar version help'."),
        };
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException("Usage: gbar version list <widget-id> [--catalog <root>]");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 2)
            throw new CliUsageException(
                "Usage: gbar version select <widget-id> <version> [--catalog <root>]");
        ...
        await output.WriteLineAsync(
            $"Selected {parsed.Positionals[0]} {requested} (disabled). Review it, then run gbar enable when ready.");
        return 0;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException(
                "Usage: gbar version rollback <widget-id> [--to <version>] [--catalog <root>]");
