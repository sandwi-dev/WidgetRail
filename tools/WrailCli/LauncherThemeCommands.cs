...
internal static class LauncherThemeCommand
{
    ...
    private static CliUsageException Usage() => new(
        "Usage: wrail launcher-theme <new|validate|preview|pack|inspect|install|list|remove> ...");

    private const string HelpText = """
        wrail launcher-theme - deterministic data-only Launcher Experience tools
        ...
        Packages are strict local data: JSON recipes, launcher-scoped WRSS, and
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException(
                "Usage: wrail launcher-theme new <Name> [--output <directory>] [--id <id>] " +
        ...
        try
        {
            ...
            await output.WriteLineAsync($"Next: wrail launcher-theme preview \"{target}\"");
            return 0;
        }
        finally
        {
...
internal static class LauncherThemeValidateCommand
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException("Usage: wrail launcher-theme validate <directory|file.wrlauncher>");
...
internal static class LauncherThemePackCommand
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException(
                "Usage: wrail launcher-theme pack <directory> [--output <file.wrlauncher>]");
...
internal static class LauncherThemeInspectCommand
{
        ...
        CancellationToken cancellationToken)
    {
        var parsed = new CommandArguments(args);
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException("Usage: wrail launcher-theme inspect <file.wrlauncher>");
...
internal static class LauncherThemePreviewCommand
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException(
                "Usage: wrail launcher-theme preview <directory|file.wrlauncher> [--output <preview.json>]");
        var inspection = await LauncherThemeSource.InspectAsync(parsed.Positionals[0], cancellationToken);
...
internal static class LauncherThemeInstallCommand
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException(
                "Usage: wrail launcher-theme install <file.wrlauncher> [--catalog <root>]");
...
internal static class LauncherThemeListCommand
{
    public static async Task<int> RunAsync(string[] args, TextWriter output)
    {
        ...
        if (parsed.Positionals.Count != 0)
            throw new CliUsageException("Usage: wrail launcher-theme list [--catalog <root>]");
        var snapshot = new WidgetRail.LauncherExperienceCatalog.LauncherExperienceCatalog(
...
internal static class LauncherThemeRemoveCommand
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (parsed.Positionals.Count != 2)
            throw new CliUsageException(
                "Usage: wrail launcher-theme remove <exact-id> <exact-version> [--catalog <root>]");
...
internal static class LauncherThemeTemplate
{
        ...
        string preset)
    {
        ...
        File.WriteAllText(Path.Combine(root, "styles", "launcher.wrss"), Style,
            new UTF8Encoding(false));
