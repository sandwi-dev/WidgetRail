    ...
    int TemplateVersion)
{
    ...
    {
        var cli = Read(typeof(CliApplication).Assembly, "wrail");
        var sdk = Read(typeof(Widget).Assembly, "WidgetSdk");
        if (cli != sdk)
            throw new CliUsageException(
                "wrail and WidgetSdk were built from different release-unit contracts. " +
        ...
        string label)
    {
        if (!metadata.TryGetValue(key, out var value) || string.IsNullOrWhiteSpace(value))
            throw new CliUsageException(
                $"The {label} assembly is missing release metadata '{key}'. Rebuild wrail and WidgetSdk from the same checkout.");
        return value;
    }
