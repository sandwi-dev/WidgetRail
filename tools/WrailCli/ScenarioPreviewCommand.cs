...
{
    internal const string DefaultManifestName = "widgetrail.scenarios.json";
        ...
        CancellationToken cancellationToken)
    {
        var executable = Path.Combine(
            Path.GetDirectoryName(typeof(CliApplication).Assembly.Location)!, "wrail.exe");
    ...
    private static string ResolveManifest(string source)
    {
        ...
        if (!File.Exists(path))
            throw new CliUsageException(
                $"Scenario manifest does not exist: {path}. Run 'wrail preview help' for the v1 format.");
