...
internal static class ControllerWidgetScaffolder
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var staging = Path.Combine(
            parent,
            $".{targetName}.wrail-stage-{Guid.NewGuid():N}");
        var published = false;
        ...
        try
        {
            ...
            var feed = Path.Combine(staging, ".widgetrail", "packages");
            ...
            if (!published && Directory.Exists(staging))
            {
                ...
                catch (Exception cleanup) when (cleanup is IOException or UnauthorizedAccessException)
                {
                    ...
                    throw new CliOperationException(
                        ...
                        "Remove that wrail-owned staging directory before retrying.",
                        failure);
                }
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!Directory.Exists(root))
            throw new CliUsageException(
                $"ControllerWidget template directory does not exist: {root}. Rebuild or reinstall wrail, or correct WRAIL_TEMPLATE_ROOT.");
        RejectReparse(root, "template root");
        var manifestPath = Path.Combine(root, "template.json");
        if (!File.Exists(manifestPath))
            throw new CliUsageException(
                $"ControllerWidget template manifest is missing: {manifestPath}. Rebuild or reinstall wrail, or correct WRAIL_TEMPLATE_ROOT.");
        ...
        foreach (var declaration in manifest.Files)
        {
            ...
            if (destination.StartsWith(".widgetrail/", StringComparison.OrdinalIgnoreCase) ||
                destination.Equals(".widgetrail", StringComparison.OrdinalIgnoreCase))
                throw new CliUsageException(
    ...
    private static TemplateManifest ParseManifest(byte[] bytes, string selectedProfile)
    {
        ...
        if (version != SupportedTemplateVersion)
            throw new CliUsageException(
                $"Template version {version} is unsupported; this wrail supports version {SupportedTemplateVersion}. Update wrail or the template as one release unit.");
    ...
    private static void RequireProperties(JsonElement element, string label, params string[] expected)
    {
        ...
        foreach (var property in element.EnumerateObject())
        {
            if (!expectedSet.Contains(property.Name))
                throw new CliUsageException(
                    $"The {label} contains unknown property '{property.Name}'. Remove it or update wrail and the template together.");
    ...
    private static CliUsageException ExistingDestination(string target) =>
        new($"Output path already exists: {target}. Choose a new path; wrail never overwrites or deletes an existing destination.");
