...
internal static partial class NewCommand
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (File.Exists(target) || Directory.Exists(target))
            throw new CliUsageException(
                $"Output path already exists: {target}. Choose a new path; wrail never overwrites or deletes an existing destination.");
        ...
        await output.WriteLineAsync(
            $"  SDK: WidgetRail.WidgetSdk {sdkPackage.Version} (local offline feed)");
...
internal static class TemplateLocator
{
    ...
    {
        var configured = Environment.GetEnvironmentVariable("WRAIL_TEMPLATE_ROOT");
        ...
        throw new CliUsageException("ControllerWidget template was not found. Set WRAIL_TEMPLATE_ROOT to its directory.");
    }
}
