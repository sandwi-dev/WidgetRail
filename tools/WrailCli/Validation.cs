...
internal static class ValidateCommand
{
    public static async Task<int> RunAsync(string[] args, TextWriter output, TextWriter error)
    {
        ...
        if (parsed.Positionals.Count != 1)
            throw new CliUsageException("Usage: wrail validate <widget-directory|manifest.json|style.wrss>");
        ...
        var files = Directory.Exists(target)
            ? Directory.EnumerateFiles(target, "*", SearchOption.AllDirectories)
                .Where(path => Path.GetFileName(path).Equals("manifest.json", StringComparison.OrdinalIgnoreCase) ||
                               Path.GetExtension(path).Equals(".wrss", StringComparison.OrdinalIgnoreCase))
                .Order(StringComparer.Ordinal)
                .ToArray()
            : [target];

        if (files.Length == 0)
            throw new CliUsageException("No manifest.json or .wrss files were found.");

        var diagnostics = new List<ToolDiagnostic>();
        foreach (var file in files)
        {
            if (Path.GetExtension(file).Equals(".wrss", StringComparison.OrdinalIgnoreCase))
                diagnostics.AddRange(WrssValidator.ValidateFile(file));
                ...
                throw new CliUsageException("Validation supports manifest.json and .wrss files.");
        }

        foreach (var warning in diagnostics.Where(item => item.Severity == WrssDiagnosticSeverity.Warning))
...
public static class WrssValidator
{
    public static IReadOnlyList<ToolDiagnostic> ValidateFile(string file)
    {
        ...
        return Diagnostics(root, WrssPackageLoader.Load(
            entry, new WrssFileSourceProvider(root)));
    }
    ...
    public static IReadOnlyList<ToolDiagnostic> Validate(string file, string source)
    {
        ...
        var root = Path.GetDirectoryName(fullFile)!;
        var entry = Path.GetFileName(fullFile);
        var package = WrssPackageLoader.Load(entry, new EntrySourceProvider(root, entry, source));
    ...
    private sealed class EntrySourceProvider(string root, string entry, string source) : IWrssSourceProvider
    {
        private readonly WrssFileSourceProvider _files = new(root);
