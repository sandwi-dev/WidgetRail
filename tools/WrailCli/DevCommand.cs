    ...
    string? ArchivePath)
{
    public static DevWidgetSource Discover(string requested)
    {
        ...
        if (File.Exists(path) &&
            Path.GetExtension(path).Equals(".wrwidget", StringComparison.OrdinalIgnoreCase))
        {
            RejectReparse(path);
...
internal sealed class DevSession : IAsyncDisposable
{
        ...
        TimeSpan? readyTimeout = null)
    {
        ...
        _output = output;
        _error = error;
        _sessionRoot = Path.Combine(Path.GetTempPath(), "WidgetRail", "wrail-dev",
...
internal static class DevGenerationBuilder
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var package = Path.Combine(generationRoot, "widget.wrwidget");
        ...
        if (!string.Equals(manifest.Entrypoint.Runtime,
            ...
            throw new CliOperationException(
                "wrail dev supports sandboxed dotnet-worker projects; full-trust applications are installed and launched only through the production catalog.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var style = Path.Combine(root, "styles", "default.wrss");
        if (File.Exists(style))
        {
            var diagnostics = WrssValidator.Validate(style,
            ...
            var firstError = diagnostics.FirstOrDefault(item => item.Severity == WrssDiagnosticSeverity.Error);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var files = DevSourceWatcher.EnumerateBounded(styles, "*.wrss", 256).ToArray();
...
internal sealed class DevSourceWatcher : IDisposable
{
        ...
        foreach (var directory in directories)
        {
            ...
            bool Accepts(string? name) => name is not null &&
                (allPackageFiles || names.Contains(name) || childDirectories.Contains(name) ||
                  acceptsProjectInput ||
                  (acceptsNewStyle && Path.GetExtension(name).Equals(".wrss", StringComparison.OrdinalIgnoreCase)));
    ...
    internal static HashSet<string> Capture(DevWidgetSource source)
    {
        ...
        foreach (var file in EnumerateBounded(Path.Combine(source.Root, "styles"), "*.wrss", 256))
            files.Add(file);
        return files;
    }
